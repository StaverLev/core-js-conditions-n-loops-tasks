...
function convertToRomanNumerals(num) {
  ...
  if (num > 10 && num < 20) {
    return romanNumbers[10] + romanNumbers[num - 10];
  }
  if (num >= 20 && num < 30) {
    return romanNumbers[20] + romanNumbers[num - 20];
  }
  if (num >= 30 && num < 40) {
    return romanNumbers[30] + romanNumbers[num - 30];
  }
  return [];
}
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < Math.floor(matrixLength / 2); i += 1) {
    for (let j = i; j < matrixLength - i - 1; j += 1) {
      ...
      resultMatrix[i][j] = resultMatrix[matrixLength - j - 1][i];
      resultMatrix[matrixLength - j - 1][i] =
        resultMatrix[matrixLength - i - 1][matrixLength - j - 1];
      resultMatrix[matrixLength - i - 1][matrixLength - j - 1] =
        resultMatrix[j][matrixLength - i - 1];
      resultMatrix[j][matrixLength - i - 1] = temp;
    }
  }
  return resultMatrix;
}
...
function sortByAsc(arr) {
  function partition(array, low, high) {
    ...
    const copyArr = arr;
    const temp = copyArr[i + 1];
    copyArr[i + 1] = arr[high];
    copyArr[high] = temp;
...
function getNearestBigger(number) {
  ...
  let result = 0;
  const arrayDigits = left.concat(right);

  for (let k = 0; k < arrayDigits.length; k += 1) {
    result = result * 10 + arrayDigits[k];
  }
